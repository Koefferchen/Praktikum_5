...
def plot_multifit( data_exp, save_as, datas_fit=None, labels_fit=None, E_of_C=None, C_of_E=None, plot_range_x=None, plot_range_y=None ):
    ...
    if( isinstance(datas_fit, list) ): 
        for i in range(len(datas_fit)):
            colors_fit                          = sns.color_palette("magma", len(datas_fit)+1)
...
data_names_NaI = [
    "NaI_BG_300s.txt",
    "NaI_Co_300s_20mm.txt",
    "NaI_Cs_300s_100mm.txt",
    "NaI_Eu_300s_200mm.txt"
]
prefix = "NaI"

channel     = np.loadtxt("../Data/"+data_names_NaI[0])[:-1 , 0]
bg          = np.loadtxt("../Data/"+data_names_NaI[0])[:-1 , 1]
counts_Co   = np.loadtxt("../Data/"+data_names_NaI[1])[:-1 , 1]
counts_Cs   = np.loadtxt("../Data/"+data_names_NaI[2])[:-1 , 1]
counts_Eu   = np.loadtxt("../Data/"+data_names_NaI[3])[:-1 , 1]
...
range_Co_1173 = [9250, 10250]
range_Co_1332 = [10500, 11500]
fitbook_Co = {
    "data_set"          : data_Co,
    "region_of_interest": [9200, 12000],
    "fit_function"      : fitfunc_gauss_double_bg,
    "params_guess"      : [*gauss_guess(data_Co, range_Co_1173), *gauss_guess(data_Co, range_Co_1332), 0, 0],
    "region_of_fit"     : None,
    "fit_density"       : 400
}

fitdata_Co, params_Co, params_Co_err, chi_Co = ultimate_fit( fitbook_Co )

fitdata_Co     = [fitdata_Co]
fitlabels_Co   = [ 
    f"1173keV, 1332keV $(\\chi^2 = {chi_Co:.1f})$"
]

#plot_multifit(data_Co_noerr, prefix+"_Co", fitdata_Co, fitlabels_Co )

params_Co_1     = np.concatenate( (params_Co[0:3], params_Co[6:]) ) 
params_Co_2     = np.concatenate( (params_Co[3:6], params_Co[6:]) ) 
params_Co_1_err = np.concatenate( (params_Co_err[0:3], params_Co_err[6:]) ) 
params_Co_2_err = np.concatenate( (params_Co_err[3:6], params_Co_err[6:]) ) 
...
#plot_multifit(data_Eu_noerr, prefix+"_Eu_13000", E_of_C=E_of_C, C_of_E=C_of_E, plot_range_x=[0, 13000])

range_Eu_121    = [1000 , 1400 ]
fitbook_Eu_121  = {
    "data_set"          : data_Eu,
...
fitdata_Eu_5000     = [fitdata_Eu_121, fitdata_Eu_244, fitdata_Eu_344]
fitdata_Eu_10000    = [fitdata_Eu_778, fitdata_Eu_964] 
fitdata_Eu_15000    = [fitdata_Eu_1408]
...
fitlabels_Eu        = fitlabels_Eu_5000 + fitlabels_Eu_10000 + fitlabels_Eu_15000

# plot_multifit(data_Eu_noerr, prefix+"_Eu_13000", fitdata_Eu, fitlabels_Eu, E_of_C=E_of_C, C_of_E=C_of_E, plot_range_x=[0, 13000])


# --------------------------- Second Energy Kalibration ---------------------------


params_Eu       = [params_Eu_121, params_Eu_244, params_Eu_344, params_Eu_778, params_Eu_964, params_Eu_1408]
params_Eu_err   = [params_Eu_121_err, params_Eu_244_err, params_Eu_344_err, params_Eu_778_err, params_Eu_964_err, params_Eu_1408_err]

energies_Eu     = [121.7817, 244.6974, 344.2785, 778.9045, 964.057, 1408.013]
energies_Eu_err = [0.0003,   0.0008,   0.0012,   0.0024,   0.005,   0.003]
...
def fitfunc_FWHM( E_gamma, alpha, beta ):
    return alpha * np.sqrt( E_gamma ) +  beta

fitbook_intrins = {
    "data_set"          : data_intrins,
    "region_of_interest": None,
    "fit_function"      : fitfunc_FWHM,
    "params_guess"      : [1 ,0],
    "region_of_fit"     : None,
    "fit_density"       : 400
}
...
def plot_FWHM(exp_data, fit_data, chi):
    ...
    save_plot = True, "../Figures/"+prefix+"_FWHM.jpg"                                     
    ultimate_plot_advanced (all_data, writtings, zoom_params, colorbar_params, extra_label, extra_xaxis, save_plot, all_sample_format_dicts, general_format_dict)

plot_FWHM(data_intrins, fitdata_intrins, chi_intrins)
...
dist_Eu, dist_Eu_err    = 200, 5        # in mm
radius_HPGe             = 50.8 / 2      # in mm
...
rel_itens_Eu            = np.array([28.53, 7.55, 26.59, 12.93, 14.51, 20.87]) /100
rel_itens_Eu_err        = np.array([0.16,  0.04, 0.20,  0.08,  0.07,  0.09]) /100
...
# --------------------------- Cobalt Aktivität ---------------------------
    ...
    # Energiekalibration ergibt negative Energien auf niedrigen Kanälen ---> schneide ab.
channel_reduced = channel[channel>200]

mu_Co1, FWHM_Co1, area_Co1, trash1, trash2 = params_Co_1
mu_Co2, FWHM_Co2, area_Co2, trash1, trash2 = params_Co_2

Energy_axis, Energy_axis_err    = E_of_C_err(channel_reduced, np.full(len(channel_reduced),0))
weight_axis         = fitfunc_eff_3(Energy_axis, *params_eff)
weight_axis_err     = None   

area_Co1_corr       = np.sum( fitfunc_gauss_bg( channel_reduced, mu_Co1, FWHM_Co1, area_Co1, 0, 0 ) / weight_axis )
area_Co2_corr       = np.sum( fitfunc_gauss_bg( channel_reduced, mu_Co2, FWHM_Co2, area_Co2, 0, 0 ) / weight_axis )
